...
import com.example.todolist.Model.NetworkWeatherRepository
    ...
    Scaffold(
        ...
        floatingActionButton = {
            Box(modifier = Modifier.fillMaxSize()) {
                ...
                DropdownMenu(
                    expanded = expanded,
                    onDismissRequest = { expanded = false }, //Note to self for future: If i tap anywhere outside the expansion of the dropdown will close
       ...
       //COLUMN THAT CONTAINS WEATHER CONTENT/API
        Column(
            ...
            Text("WEATHER", fontSize = 18.sp, fontWeight = FontWeight.Bold, modifier = Modifier.padding(start = 135.dp))
                ...
                Card(
                    ...
                    Column(
                        ...
                        Row(
                            ...
                            Text(weather.location, fontSize = 30.sp, fontWeight = FontWeight.Bold)
                            Spacer(modifier = Modifier.width(16.dp))
                            Text("${state.weatherInfo.current.temp}", fontSize = 25.sp, fontWeight = FontWeight.Bold)
                        ...
                        Row(
                            ...
                            Text("Sunrise - ${state.astronomyInfo.astronomy.astro.sunrise}", fontSize = 15.sp, fontWeight = FontWeight.Bold)
                            Spacer(Modifier.height(12.dp))
                            Text("Sunset - ${state.astronomyInfo.astronomy.astro.sunset}", fontSize = 15.sp, fontWeight = FontWeight.Bold)
                        }
                    }
                }
            }
            ...
            Text("TASKS TO DO", fontSize = 18.sp, fontWeight = FontWeight.Bold)
            Spacer(Modifier.height(8.dp))
                    ...
                    listOf("Low", "Medium", "High").forEach {
                        AssistChip(
                            ...
                            colors = AssistChipDefaults.assistChipColors(
                                containerColor = if (selectedPriority == it) Color.Gray else Color.White
                            )
                        )
                    }
                }
                ...
                LazyRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    items(fullTitleList) { title -> //pull data object of list of titles
                        AssistChip( //assists chips better for when i have multiple option and want to change appearance of the option i have selected; assists chips👍🏿
                            ...
                            onClick = { selectedTitle = title }, label = { Text(title) }, //this assist chip will store title name
                            colors = AssistChipDefaults.assistChipColors(
                                //so if the assist chip is select it will be grey else it should/will be white
                                containerColor = if (selectedTitle == title) Color.Gray else Color.White
                            )
                        )
                    }
                }
...
fun DatePickerModal(
   ...
   // var selectedDate by remember { mutableStateOf(InitialDate) }
    ...
    AlertDialog(//alert dialog is new from material, giving it a try
        // alert dialog is similar to prompt pop up or showDialog in c# or is it java, this will be better to prompt users to select date better than DatePicker
    ...
    Card(
            ...
            contentAlignment = Alignment.Center)
        {
            Column(
                ...
                Text("To Do: ${summary.taskCount} ", fontSize = 15.sp)
...
data class weatherAPI( val location: String, val sunrise: String, val sunset: String, val temperature: String)
